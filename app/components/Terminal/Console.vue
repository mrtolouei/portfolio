...
`
    ...
    <div v-if="isShuttingDown" class="absolute inset-0 bg-black flex flex-col justify-center items-center text-gray-100 text-sm space-y-2">
      <div class="text-left whitespace-pre w-[300px]">
        <div v-for="msg in shutdownMessages" :key="msg">{{ msg }}</div>
      </div>
      <div v-if="showRestartPrompt" class="mt-4 text-gray-400">Press ENTER to restart</div>
    </div>
  </div>
</template>
...
watch([isRunning, isShuttingDown], async ([running, shuttingDown]) => {
